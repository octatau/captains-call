...
4a. **Recency is a HARD GATE.** The authoritative data must be from within the **last 2 calendar years** (e.g. in 2026, only 2024–2026 data qualifies). This applies to the underlying ranking, not just when it was republished. Reject any topic whose best available source is older than 2 years — do not ship it, do not ask for an exception. Reframe to a metric that naturally refreshes (e.g. "Most Watched X in 2025", "Top X by 2024 Revenue", "Longest-Running X by Seasons Aired") or drop the topic. Retrospective "of all time" lists are only acceptable when (a) the ranking itself was published within the last 2 years AND (b) the source refreshes it periodically (e.g. Billboard, Forbes annual lists). The "Data as of Month Year" entry in the sources array must also fall within this window.
