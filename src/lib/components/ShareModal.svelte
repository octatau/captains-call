...
>
	<!-- Modal Content -->
	<div
		class="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-md w-full p-6 relative animate-scaleIn"
	>
		<!-- Close Button -->
		<button
			onclick={onClose}
		...
		<!-- Header -->
		<div class="mb-6">
			<h2 class="text-2xl font-bold {theme.neutral.textStrong} mb-2">Share Your Results</h2>
			<div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-4 {theme.neutral.text} font-mono text-sm whitespace-pre-line">
				{shareText}
			</div>
		</div>
		...
		<div class="mb-6">
			<h3 class="text-sm font-semibold {theme.neutral.textStrong} mb-3">Share as Image</h3>
			<div class="grid grid-cols-2 gap-3">
				<!-- Download Image -->
				<button
					onclick={handleDownloadImage}
					disabled={generatingImage}
					class="flex items-center justify-center gap-2 p-3 rounded-lg {theme.primary.bg} hover:bg-pearl-aqua-700 dark:hover:bg-pearl-aqua-600 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
				>
					{#if generatingImage}
						<div class="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent"></div>
					{:else}
						<Icon src={ArrowDownTray} size="20" />
					{/if}
				...
				{:else}
					...
					>
						{#if generatingImage}
							<div class="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent"></div>
						{:else}
							<Icon src={Photo} size="20" />
						{/if}
						<span class="font-medium">{copied ? 'Copied!' : 'Copy Image'}</span>
					</button>
				{/if}
			</div>
		</div>

		<div class="border-t {theme.neutral.border} my-4"></div>

		<!-- Share Options -->
		<div class="space-y-3">
			<h3 class="text-sm font-semibold {theme.neutral.textStrong} mb-3">Share on Social Media</h3>
			<p class="text-xs {theme.neutral.text} mb-3">Click a platform below - the image will be copied to your clipboard, then paste it in your post!</p>
			<!-- Copy to Clipboard -->
			<button
				onclick={copyToClipboard}
				class="w-full flex items-center gap-3 p-3 rounded-lg border-2 {theme.neutral.border} hover:border-pearl-aqua-500 dark:hover:border-pearl-aqua-400 transition-all {theme.card.bg}"
			>
				<Icon src={ClipboardDocument} size="24" class="{theme.primary.text}" />
				<span class="flex-1 text-left font-medium {theme.neutral.textStrong}">
			...
			{#if navigator.share}
				<button
					...
					class="w-full flex items-center gap-3 p-3 rounded-lg border-2 {theme.neutral.border} hover:border-pearl-aqua-500 dark:hover:border-pearl-aqua-400 transition-all {theme.card.bg}"
				>
					<Icon src={ShareIcon} size="24" class="{theme.primary.text}" />
					<span class="flex-1 text-left font-medium {theme.neutral.textStrong}">Share via...</span>
				</button>
			{/if}
			...
			<div class="grid grid-cols-2 gap-3">
				...
				<button
					...
					class="flex items-center justify-center gap-2 p-3 rounded-lg bg-black hover:bg-gray-800 text-white transition-colors"
				>
				...
				<button
					...
					class="flex items-center justify-center gap-2 p-3 rounded-lg bg-[#1877F2] hover:bg-[#166FE5] text-white transition-colors"
				>
					<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
				...
				<button
					...
					class="flex items-center justify-center gap-2 p-3 rounded-lg bg-[#0A66C2] hover:bg-[#095196] text-white transition-colors"
				>
					<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
				...
				<button
					...
					class="flex items-center justify-center gap-2 p-3 rounded-lg bg-[#FF4500] hover:bg-[#E03D00] text-white transition-colors"
				>
					<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
			...
			<button
				...
				class="w-full flex items-center gap-3 p-3 rounded-lg border-2 {theme.neutral.border} hover:border-pearl-aqua-500 dark:hover:border-pearl-aqua-400 transition-all {theme.card.bg}"
			>
				<svg class="w-6 h-6 {theme.primary.text}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
				</svg>
				<span class="flex-1 text-left font-medium {theme.neutral.textStrong}">Share via Email</span>
			</button>
		</div>
	</div>
</div>
